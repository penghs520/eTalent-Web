export default class OrgChart extends HTMLElement {
  ...
  _attachRel(data, flags) {
    let childKey = this.options.childTitle;
    data.relationship = flags + (data[childKey] && data[childKey].length > 0 ? 1 : 0);
    if (data[childKey]) {
      for (let item of data[childKey]) {
        this._attachRel(item, '1' + (data[childKey].length > 1 ? 1 : 0));
      }
    }
    return data;
  }
